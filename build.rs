...
fn main() {
    ...
    let dbus_parsec_control_server_gen = dbus_codegen::generate(&dbus_parsec_control_iface, &codegen_opts_server)
        .expect("Error generting control server code");
    std::fs::write(out_dir.join("dbus_parsec_control_server.rs"), dbus_parsec_control_server_gen)
        .expect("Error writing control server code");

    let dbus_parsec_control_client_gen = dbus_codegen::generate(&dbus_parsec_control_iface, &codegen_opts_client)
        .expect("Error generting control client code");
    std::fs::write(out_dir.join("dbus_parsec_control_client.rs"), dbus_parsec_control_client_gen)
